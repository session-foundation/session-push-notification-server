...
def load_config(conf_ini: str | None = None) -> spns_hivemind.Config:
    if conf_ini:
        if not os.path.exists(conf_ini):
            raise RuntimeError(f"Given config ini file {conf_ini} does not exist")
    elif "SPNS_CONFIG" in os.environ:
    ...
    if not conf_ini:
        ...
        if not os.path.exists(conf_ini):
            raise RuntimeError(
                "spns.ini does not exist; either create it or specify a path using the conf_ini= argument, "
                "or the SPNS_CONFIG=... environment variable to specify an alternate config file"
            )
    ...
    setting_map = {
        ...
        "hivemind": {
            ...
            "listen_curve_admin": (
                ...
                lambda x: re.search(r"^(?:[a-fA-F0-9]{64}\s+)*[a-fA-F0-9]{64}\s*$", x),
    ...
    for s in cp.sections():
        if s == "keys":
            ...
            for opt in cp["log"]:
                if opt == "level":
                    spns_hivemind.logger.set_level(cp["log"][opt])
                elif opt.startswith("level-") and len(opt) > 6:
                    logger.warning(
                        f"{opt} = ... is deprecated; use a compound level=... instead"
                    )
                    spns_hivemind.logger.set_level(f'{opt[6:]}={cp["log"][opt]}')
                else:
                    logger.warning(
